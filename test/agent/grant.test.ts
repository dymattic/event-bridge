...
beforeEach(() => setUrl('https://development.rave.page/desktop/bridge?target=extension'));
afterEach(() => vi.useRealTimers());

describe('awaitGrant', () => {
  it('posts hello on start, replies hello to bridge-ready, resolves on grant', async () => {
    const spy = vi.spyOn(window, 'postMessage');
    const p = awaitGrant(5_000);
    ...
    post({ type: 'rave-page:desktop-grant', version: 1, code: 'C1', api: 'https://development.api.rave.page' });
    await expect(p).resolves.toEqual({ code: 'C1', api: 'https://development.api.rave.page' });
  });

  it('resolves even when the grant arrives before any bridge-ready', async () => {
    const p = awaitGrant(5_000);
    post({ type: 'rave-page:desktop-grant', code: 'C2', api: 'https://development.api.rave.page' });
  ...
  it('ignores wrong-origin grants', async () => {
    ...
    post({ type: 'rave-page:desktop-grant', code: 'GOOD', api: 'https://development.api.rave.page' });
    await expect(p).resolves.toMatchObject({ code: 'GOOD' });
  });

  it('rejects TIMEOUT after timeoutMs', async () => {
    vi.useFakeTimers();
    const p = awaitGrant(1_000);
    const assertion = expect(p).rejects.toMatchObject({ code: 'TIMEOUT' });
    await vi.advanceTimersByTimeAsync(1_000);
    await assertion;
  });

  it('rejects UNSUPPORTED off the rave.page dev origin', async () => {
    setUrl('https://vrcpop.com/');
    await expect(awaitGrant(1_000)).rejects.toMatchObject({ code: 'UNSUPPORTED' });
  });
});
